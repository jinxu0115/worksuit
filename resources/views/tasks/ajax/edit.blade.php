                            ...
                            || ($changeStatusPermission == 'both' && (in_array(user()->id, $taskUsers) || $task->added_by == user()->id || $task->project->project_admin == user()->id))
                                ...
                                <div class="col-lg-3 col-md-6">
                                    @if ($task->board_column_id === $waitingApprovalTaskBoardColumn->id)
                                        ...
                                        @endforeach
                                    @else
                                    <x-forms.label fieldId="board_column_id" :fieldLabel="__('app.status')" class="mt-3">
                                    </x-forms.label>
                                    <x-forms.input-group>
                                        <select class="form-control flex select-picker" name="board_column_id"
                                            id="board_column_id" data-live-search="true" data-size="8" {{$task->board_column_id === $waitingApprovalTaskBoardColumn->id ? 'disabled' : ''}}>
                                            @foreach ($taskboardColumns as $item)
                                                @php
                                        ...
                                        <x-slot name="append">
                                            <button id="createStatus" type="button"
                                                class="btn btn-outline-secondary border-grey"
                                                data-toggle="tooltip" data-original-title="{{ __('modules.taskStatus.addStatus') }}">@lang('app.add')</button>
                                        </x-slot>
                                    </x-forms.input-group>
                                    @endif
                                </div>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#createStatus').click(function() {
            const url = "{{ route('task-status.create') }}";
            $(MODAL_XL + ' ' + MODAL_HEADING).html('...');
            $.ajaxModal(MODAL_XL, url);
        });
