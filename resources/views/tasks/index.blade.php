...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="d-grid d-lg-flex d-md-flex align-items-center action-bar mt-3">
            <lable class="label label-info">Table Column Settings:</lable>
            <select id="settingTaskTable" name="settingTaskTable[]" class="select-picker mx-md-3 mx-0 my-md-0 my-2" multiple>
                @foreach (json_decode($taskColumns, true) as $key => $value)
                    @if(in_array($key, ['id', 'user_id', 'created_at', 'updated_at']))
                        @continue;
                    @endif
                    @if(!checkCompanyAllowd($key))
                        @continue;
                    @endif
                    <option value="{{ $key }}" {{ $value == 1 ? "selected" : "" }}>{{ ucwords(str_replace('_', ' ', $key)) }}</option>
                @endforeach
            </select>
            <x-forms.button-primary id="apply_task_table_column" class="mr-md-3 mr-0 float-left" icon="cog">
                @lang('modules.tasks.applySetting')
...
@push('scripts')
    ...
    <script>
        ...
        $('#apply_task_table_column').on('click', function() {
            var url = "{{ route('tasks.setTableColumn') }}";
            var token = '{{ csrf_token() }}';
            var tableColumnSettings = $('#settingTaskTable').val()
            $.easyAjax({
                url: url,
                blockUI: true,
                type: "POST",
