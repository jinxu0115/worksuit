...
@push('scripts')
    ...
    <script>
        ...
        $(document).on('click', '.applyTaskColumnAllow', function(){
            const token = "{{ csrf_token() }}";
            const selectedOptions = $(this).closest('div').find('select').val();
            $.easyAjax({
                type: 'POST',
                blockUI: true,
                url: "{{route('superadmin.companies.allowTaskColumn')}}",
                data: {'_token': token, 'task_company_column_allow_id': $(this).attr('data-id'), data: selectedOptions},
                success: function (response) {
                    if (response.status === "success") {
