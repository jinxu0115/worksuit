...
class CompanyController extends AccountBaseController
{
    ...
    public function allowTaskColumn(Request $request){
        $tableColumnSettings = $request->data;
        $taskCompanyColumnAllow = TaskCompanyColumnAllow::where('id', $request->task_company_column_allow_id)->first();
    
        foreach ($taskCompanyColumnAllow->getAttributes() as $key => $value) {
            if (in_array($key, ['id', 'company_id', 'created_at', 'updated_at'])) {
                continue;
            }
            if(!isset($tableColumnSettings)){
                $taskCompanyColumnAllow[$key] = false;
            } else {
                $taskCompanyColumnAllow[$key] = in_array($key, $tableColumnSettings) ? true : false;
            }
        }
        $taskCompanyColumnAllow->save();
