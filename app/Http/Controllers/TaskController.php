...
class TaskController extends AccountBaseController
{
    ...
    public function index(TasksDataTable $dataTable)
    {
        ...
        if (!request()->ajax()) {
            ...
            $this->taskColumns = json_encode(TaskColumns::where('user_id', user()->id)->first());
    ...
    public function setTableColumn(Request $request) {
        $tableColumnSettings = $request->tableColumnSettings;
        $taskColumn = TaskColumns::where('user_id', user()->id)->first();
    
        foreach ($taskColumn->getAttributes() as $key => $value) {
            ...
            if(!isset($tableColumnSettings)){
                ...
                $taskColumn[$key] = in_array($key, $tableColumnSettings) ? true : false;
            }
        }
