...
use App\Models\TaskCompanyColumnAllow;
use Carbon\Carbon;
...
class CompanyDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->addColumn('task_column_allow', function ($row) {
            $taskColumnAllow = TaskCompanyColumnAllow::where('company_id', $row->id)->first();
            $html = '';
            if(!empty($taskColumnAllow)){
                $html = '<div class="d-flex align-items-center allowTaskColumn"><select name="allowTaskColumn[]" class="select-picker mx-md-3 mx-0 my-md-0 my-2" multiple>';
                foreach ($taskColumnAllow->getAttributes() as $key => $value) {
                    if(in_array($key, ['id', 'company_id', 'created_at', 'updated_at'])) continue;
                    $html .= '<option value="' . $key . '" ' . ($value == 1 ? 'selected' : '') . '>' . ucwords(str_replace('_', ' ', $key)) . '</option>';
                }
                $html .= '</select><button class="btn btn-primary applyTaskColumnAllow" data-id="' . $taskColumnAllow->id . '">Apply</button></div>';
            }
            return $html;
        });
        ...
        $datatables->rawColumns(array_merge(['company_name', 'action', 'status', 'package', 'details', 'nextpaymentdate', 'task_column_allow'], $customFieldColumns));
        ...
        return $this->setBuilder('companies-table')
            ->parameters([
                ...
                'fnDrawCallback' => 'function( oSettings ) {
                    $(".allowTaskColumn select").each(function() {
                        $(this).selectpicker();
                    });
                  $("body").tooltip({
    ...
    protected function getColumns()
    {
        $data = [
            ...
            'task_column_allow' => ['data' => 'task_column_allow', 'name' => 'task_column_allow', 'title' =>'Task Column Allow', 'orderable' => false],
