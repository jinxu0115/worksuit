...
class TasksDataTable extends BaseDataTable
{
    ...
    public function dataTable($query)
    {
        ...
        $datatables->editColumn('priority', function($row) {
            ...
            if($row->priority == "high"){
                $priority =  '<span class="text-danger"><i class="fa fa-circle mr-2" style="color: #dd0000"></i>High</span>';
            }
            if($row->priority == "medium"){
                $priority =  '<span class="text-danger"><i class="fa fa-circle mr-2" style="color: #ffc202"></i>Medium</span>';
            }
            if($row->priority == "low"){
                $priority =  '<span class="text-danger"><i class="fa fa-circle mr-2" style="color: #0a8a1f"></i>Low</span>';
            }
            return $priority;
        });
    ...
    {
        $tableColumn = TaskColumns::where('user_id', user()->id)->first();
        ...
        if($tableColumn->code == 0 || !checkCompanyAllowd('code')) unset($data[__('modules.taskCode')]);
        if($tableColumn->timer == 0 || !checkCompanyAllowd('timer')) unset($data[__('app.timer') . ' ']);
        if($tableColumn->task == 0 || !checkCompanyAllowd('task')) unset($data[__('app.task')]);
        if($tableColumn->completed_on == 0 || !checkCompanyAllowd('completed_on')) unset($data[__('app.completedOn')]);
        if($tableColumn->start_date == 0 || !checkCompanyAllowd('start_date')) unset($data[__('app.startDate')]);
        if($tableColumn->due_date == 0 || !checkCompanyAllowd('due_date')) unset($data[__('app.dueDate')]);
        if($tableColumn->estimated_time == 0 || !checkCompanyAllowd('estimated_time')) unset($data[__('modules.tasks.estimateTime')]);
        if($tableColumn->hours_logged == 0 || !checkCompanyAllowd('hours_logged')) unset($data[__('modules.employees.hoursLogged')]);
        if($tableColumn->assigned_to == 0 || !checkCompanyAllowd('assigned_to')) unset($data[__('modules.tasks.assignTo')]);
